...
public: 
	...
	{ 
		...
		//observer->updata(temperature_, humidity_, pressure_); 
	} 
	void removeObject(shared_ptr<Observer> observer) 
	{ 
		observersList_.remove(observer); 
	}
	void changed(float temperature, float humidity, float pressure)
	{
		temperature_    = temperature; 
		humidity_        = humidity; 
		pressure_        = pressure; 
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	wd->registerObject(observer_1); 
	wd->registerObject(observer_2); 
	wd->registerObject(observer_3); 
	wd->changed(100, 110, 120);
	getchar();
	return 0; 
